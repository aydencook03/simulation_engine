...
The goal is to then find a position change for each particle $\Delta \vec{x}_i$ such that the constraint becomes satisfied. XPBD does this by using an approximate implicit integration step where the constraint function is linearized. A detailed derivation can be found in @macklin2019small.
